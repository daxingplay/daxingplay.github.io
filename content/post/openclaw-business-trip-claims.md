...
截至写这篇文章，我自己的这个项目已经有 200 多个 commit、180 多个文件、4 万多行内容。有意思的是，前期我还会认真看 AI 每个 commit 改了什么，后面迭代速度实在太快，已经变成“看测试、看审计结果、看最终产物”。这也是我对 agent 编程感受变化很大的地方：人不再逐行控制所有实现，而是在更高层控制目标、边界和验证方式。
...
override 则是另一种补丁：当某个真实 event 的 location 或日期不够标准时，我会在配置里补一条规则，告诉 Trip Planner 这条 event 应该怎么解释。比如把一个模糊的会议地点映射成明确城市和具体会议的地点，或者指定某一天应该归到某次 trip。
